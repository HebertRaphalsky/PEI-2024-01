package br.org.rfdouro.demorest.util;
  ...
  */
  public boolean verificaAccesso(String token, String permissao) {
    try {
        ...
        throw new RuntimeException("Permissão não encontrada no token");
    } catch (Exception e) {
        // Registre a exceção original para diagnóstico
        e.printStackTrace(); // Ou use um logger para registrar a exceção
        throw new RuntimeException("Erro ao verificar acesso com o token");
    }
}
